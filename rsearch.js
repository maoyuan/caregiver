...
$(document).ready(function() {
 ...
 $(function() {
  ...
  $("#rsearch_restriction").on("change",function(){
        $.getJSON("./data.json", function(json) {
            $("table").empty();
            $("#popup").empty();
            var keywords = ["kale", "spinach", "brussels sprouts", "parsley","collard greens", "mustard greens","chard", "green tea","cranberry juice", "alcohol"];
            for(var i = 0; i < json.length; i++){
                var imgname = "img/" + json[i].image;
                var recipename = json[i].name;
                var ingredients = json[i].ingredients;
                ...
                for(var k = 0; k < keywords.length; k++ ){
                    if(ingredients.indexOf(keywords[k]) > -1 || recipename.indexOf(keywords[k]) > -1){
                        flag = 1;
                        break;
                    }
                }
